...
pragma solidity ^0.8.17 ^0.8.18 ^0.8.27 ^0.8.4;
...
// lib/filecoin-solidity/contracts/v0.8/utils/Errors.sol
...
library Errors {
    ...
    function revertOnError(int256 exit_code) internal pure {
        ...
        if (exit_code == USR_ILLEGAL_ARGUMENT)
            description = "USR_ILLEGAL_ARGUMENT";
        if (exit_code == USR_NOT_FOUND) description = "USR_NOT_FOUND";
        if (exit_code == USR_FORBIDDEN) description = "USR_FORBIDDEN";
        if (exit_code == USR_INSUFFICIENT_FUNDS)
            description = "USR_INSUFFICIENT_FUNDS";
        if (exit_code == USR_ILLEGAL_STATE) description = "USR_ILLEGAL_STATE";
        if (exit_code == USR_SERIALIZATION) description = "USR_SERIALIZATION";
        if (exit_code == USR_UNHANDLED_MESSAGE)
            description = "USR_UNHANDLED_MESSAGE";
        if (exit_code == USR_UNSPECIFIED) description = "USR_UNSPECIFIED";
        if (exit_code == USR_ASSERTION_FAILED)
            description = "USR_ASSERTION_FAILED";

        if (
            exit_code > USR_ASSERTION_FAILED &&
            exit_code < START_FOR_ACTOR_SPECIFIC_EXIT_CODES
        ) description = "Reserved exit code";

        if (exit_code >= START_FOR_ACTOR_SPECIFIC_EXIT_CODES)
            description = "Actor specific exit code";
...
library BigIntCBOR {
    ...
    function deserializeBigInt(
        ...
        require(
            raw[0] == 0x00 || raw[0] == 0x01,
            "Invalid sign byte: must be 0x00 or 0x01"
        );
...
library Misc {
    ...
    function getFilActorIdSize(
        CommonTypes.FilActorId value
    ) internal pure returns (uint256) {
        uint64 val = CommonTypes.FilActorId.unwrap(value);
        return getPrefixSize(uint256(val));
    }

    function getChainEpochSize(
        CommonTypes.ChainEpoch value
    ) internal pure returns (uint256) {
...
library Actor {
    ...
    function callByIDReadOnly(
        ...
        function(
            ...
            uint256,
            bool
        ) internal view returns (int256, bytes memory) callFn;
        function(
            CommonTypes.FilActorId,
            uint256,
            uint64,
            ...
            uint256,
            bool
        ) internal returns (int256, bytes memory) helper = callByID;
        assembly {
            callFn := helper
        }
    ...
    function readRespData(
        bytes memory raw_response
    ) internal pure returns (int256, bytes memory) {
        (int256 exit, uint64 return_codec, bytes memory return_value) = abi
            .decode(raw_response, (int256, uint64, bytes));
        ...
        if (return_codec == Misc.NONE_CODEC) {
            ...
            return_codec == Misc.CBOR_CODEC ||
            return_codec == Misc.DAG_CBOR_CODEC
        ) {
            if (return_value.length == 0) {
                revert InvalidResponseLength();
            }
...
library FilAddresses {
    ...
    function toEthAddress(
        CommonTypes.FilAddress memory addr
    ) internal pure returns (address) {
        if (
            addr.data[0] != CommonTypes.PROTOCOL_DELEGATED ||
            addr.data[1] != CommonTypes.EAM_ID ||
            addr.data.length !=
            CommonTypes.PROTOCOL_DELEGATED_EAM_ADDRESS_LENGTH
        ) {
    ...
    function fromActorID(
        uint64 actorID
    ) internal pure returns (CommonTypes.FilAddress memory) {
        Buffer.buffer memory result = Leb128.encodeUnsignedLeb128FromUInt64(
            actorID
        );
        return
            CommonTypes.FilAddress(
                abi.encodePacked(CommonTypes.PROTOCOL_ID, result.buf)
            );
    }
    ...
    function validate(
        ...
        if (addr.data[0] == CommonTypes.PROTOCOL_ID) {
            return (addr.data.length >
                CommonTypes.MIN_PROTOCOL_ID_ADDRESS_LENGTH &&
                addr.data.length <= CommonTypes.MAX_PROTOCOL_ID_ADDRESS_LENGTH);
        } else if (
            addr.data[0] == CommonTypes.PROTOCOL_SECP256K1 ||
            addr.data[0] == CommonTypes.PROTOCOL_ACTOR
        ) {
            return
                addr.data.length ==
                CommonTypes.PROTOCOL_SECP256K1_ACTOR_ADDRESS_LENGTH;
...
library FilecoinCBOR {
    ...
    function writeCid(
        ...
        buf.buf.appendUint8(
            uint8(((MAJOR_TYPE_TAG << 5) | PAYLOAD_LEN_8_BITS))
        );
    ...
    function readCid(
        ...
        require(
            maj == MAJOR_TYPE_TAG,
            "expected major type tag when parsing cid"
        );
    ...
    function writeChainEpoch(
        CBOR.CBORBuffer memory buf,
        CommonTypes.ChainEpoch id
    ) internal pure {
    ...
    function writeDealLabel(
        ...
        label.isString
            ? buf.writeString(string(label.data))
            : buf.writeBytes(label.data);
    }
    ...
    function readDealLabel(
        ...
        return (
            CommonTypes.DealLabel(slice, maj == MajTextString),
            byteIdx + len
        );
    }
}

// lib/filecoin-solidity/contracts/v0.8/cbor/DataCapCbor.sol
/*******************************************************************************
 *   (c) 2022 Zondax AG
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
...
library DataCapCBOR {
    ...
    /// @return cbor serialized data as bytes
    function serializeGetAllowanceParams(
    ...
    /// @return cbor serialized data as bytes
    function serializeTransferParams(
    ...
    function deserializeTransferReturn(
        bytes memory rawResp
    ) internal pure returns (DataCapTypes.TransferReturn memory ret) {
        uint byteIdx = 0;
        uint len;
        bytes memory tmp;
    ...
    /// @return cbor serialized data as bytes
    function serializeTransferFromParams(
    ...
    function deserializeTransferFromReturn(
        bytes memory rawResp
    ) internal pure returns (DataCapTypes.TransferFromReturn memory ret) {
        uint byteIdx = 0;
        uint len;
        bytes memory tmp;
    ...
    /// @return cbor serialized data as bytes
    function serializeIncreaseAllowanceParams(
    ...
    /// @return cbor serialized data as bytes
    function serializeDecreaseAllowanceParams(
    ...
    /// @return cbor serialized data as bytes
    function serializeBurnFromParams(
    ...
    function deserializeBurnFromReturn(
        bytes memory rawResp
    ) internal pure returns (DataCapTypes.BurnFromReturn memory ret) {
        uint byteIdx = 0;
        uint len;
        bytes memory tmp;
...
// lib/filecoin-solidity/contracts/v0.8/DataCapAPI.sol
/*******************************************************************************
 *   (c) 2022 Zondax AG
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
...
library DataCapAPI {
    ...
    function totalSupply()
        internal
        view
        returns (int256, CommonTypes.BigInt memory)
    {
        bytes memory raw_request = new bytes(0);
    ...
    function balance(
        CommonTypes.FilAddress memory addr
    ) internal view returns (int256, CommonTypes.BigInt memory) {
    ...
    function transfer(
        DataCapTypes.TransferParams memory params
    ) internal returns (int256, DataCapTypes.TransferReturn memory) {
    ...
    function transferFrom(
        DataCapTypes.TransferFromParams memory params
    ) internal returns (int256, DataCapTypes.TransferFromReturn memory) {
    ...
    function increaseAllowance(
        DataCapTypes.IncreaseAllowanceParams memory params
    ) internal returns (int256, CommonTypes.BigInt memory) {
    ...
    function revokeAllowance(
        CommonTypes.FilAddress memory operator
    ) internal returns (int256, CommonTypes.BigInt memory) {
    ...
    function burnFrom(
        DataCapTypes.BurnFromParams memory params
    ) internal returns (int256, DataCapTypes.BurnFromReturn memory) {
        ...
        return (
            exit_code,
            DataCapTypes.BurnFromReturn({
                balance: CommonTypes.BigInt({val: hex"00", neg: false}),
                allowance: CommonTypes.BigInt({val: hex"00", neg: false})
            })
        );
    }
}
